...
#刷新dnsmasq远程脚本地址
update_gfw_too_dnsmasq_sh='https://ghproxy.com/https://raw.githubusercontent.com/zhudan/gfwlist2dnsmasq/master/gfwlist2dnsmasq.sh'
...
write_regular_cron_job(){
	...
	start_regular_subscribe_day(){
		remove_regular_subscribe
		cru a regular_subscribe ${msrm} ${msrh}" * * * /bin/sh /koolshare/scripts/clash_regular_update.sh"
		echo_date "将于每日的${msrh}时${msrm}分重新订阅" >> $LOG_FILE
	}
	start_regular_subscribe_week(){
		remove_regular_subscribe
		cru a regular_subscribe ${msrm} ${msrh}" * * "${msrw}" /bin/sh /koolshare/scripts/clash_regular_update.sh"
		echo_date "将于每周${msrw}的${msrh}时${msrm}分重新订阅" >> $LOG_FILE
	}
	start_regular_subscribe_month(){
		remove_regular_subscribe
		cru a regular_subscribe ${msrm} ${msrh} ${msrd}" * * /bin/sh /koolshare/scripts/clash_regular_update.sh"
	...
	start_regular_subscribe_mhour(){
		...
		if [ "$msrm_2" == "2" ] || [ "$msrm_2" == "5" ] || [ "$msrm_2" == "10" ] || [ "$msrm_2" == "15" ] || [ "$msrm_2" == "20" ] || [ "$msrm_2" == "25" ] || [ "$msrm_2" == "30" ]; then
			cru a regular_subscribe "*/"${msrm_2}" * * * * /bin/sh /koolshare/scripts/clash_regular_update.sh"
			echo_date "将每隔${msrm_2}分钟重新订阅" >> $LOG_FILE
		fi
		if [ "$msrm_2" == "1" ] || [ "$msrm_2" == "3" ] || [ "$msrm_2" == "6" ] || [ "$msrm_2" == "12" ]; then
			cru a regular_subscribe "0 */"${msrm_2} "* * * /bin/sh /koolshare/scripts/clash_regular_update.sh"
...
set_Tolerance(){
	...
	if [ "$intervalbox" == "1" ]; then
		...
		echo_date "自定义测速时间值:$interval" >> $LOG_FILE
		sed -ri "s/(interval:)[^\"]*/interval: $interval/" $yamlpath
	else
		echo_date "未定义测速时间值，保持默认" >> $LOG_FILE
...
update_dnsmasq_gfw_dns_ipt(){
	...
	echo_date 已增加198.18.0.0/16 iptables配置 >> $LOG_FILE
}

#fake-ip & 后置dns, 更新dnsmasq的gfw域名列表并且指向到clash的dns
gen_gfw_too_dnsmasq(){
	echo_date dnsmasq分流方案介绍：由dnsmasq负责解析内网IP，根据gfw列表转发至clash dns，clash返回fake-ip，客户端发起针对fake-ip访问，clash送至远端解析防止污染
	if [ "$dnshijacksel" != "front" ] && [ "$dnsplan" == "fi" ]; then
		echo_date 已设置DNS后置并且已启用fake-ip，由dnsmasq转发gfw域名到clash dns端口进行解析，开始下载执行脚本 >> $LOG_FILE
		curl -s "$update_gfw_too_dnsmasq_sh" -m 10 --connect-timeout 10 | bash -s gen 127.0.0.1 $dnslistenport
	else
		echo_date 你已关闭dnsmasq dns级无污染分流方案；如要启用请打开DNS后置和fake-ip，关闭高级模式中的代理路由DNS、清除路由自定义DNS、透明代理即可 >> $LOG_FILE
	fi
}
...
del_dnsmasq_gfw(){
	...
	curl -s "$update_gfw_too_dnsmasq_sh" -m 10 --connect-timeout 10 | bash -s del
}
				...
				for nokpacl in $nokpacl_nu; do
					...
					#	iptables -t nat -A PREROUTING $(factor $macaddr "-m mac --mac-source") -p tcp $(factor $ports "-m multiport --dport") -j merlinclash
						...
						if [ "$tproxymode" == "udp" ]; then
							...
							iptables -t mangle -I PREROUTING $(factor $macaddr "-m mac --mac-source") -i pptp+ -p udp $(factor $ports "-m multiport --dport") -j merlinclash_PREROUTING
							iptables -t mangle -I PREROUTING $(factor $macaddr "-m mac --mac-source") -i $interface -p udp $(factor $ports "-m multiport --dport") -j merlinclash_PREROUTING
			...
			#2.白名单内再放行
				...
				#	iptables -t nat -A merlinclash -p tcp -j merlinclash_CHN
					if [ "$tproxymode" == "udp" ]; then
						...
						iptables -t mangle -A PREROUTING -m set --match-set lan_whitelist src -i $interface -p udp  -j merlinclash_PREROUTING
						iptables -t mangle -A PREROUTING -m set --match-set lan_whitelist src -i pptp+ -p udp  -j merlinclash_PREROUTING
					fi
				else
					...
					if [ "$merlinclash_nokpacl_default_mode" == "1" ]; then #剩余主机访问全端口通过clash
						...
						if [ "$cirswitch" == "1" ]; then
							...
							if [ "$dnshijacksel" == "front" ]; then
								iptables -t nat -I PREROUTING -p udp --dport 53 -i $interface -j REDIRECT --to-port "${dnslistenport}" >/dev/null 2>&1
								iptables -t nat -I PREROUTING -p udp --dport 53 -i pptp+ -j REDIRECT --to-port "${dnslistenport}" >/dev/null 2>&1
								iptables -t nat -I PREROUTING -p udp --dport 53 -i tun+ -j REDIRECT --to-port "${dnslistenport}" >/dev/null 2>&1
					...
					if [ "$merlinclash_nokpacl_default_mode" == "1" ]; then #剩余主机访问指定端口通过clash
						...
						if [ "$cirswitch" == "1" ]; then
							...
							if [ "$tproxymode" == "udp" ]; then
								...
								iptables -t mangle -A PREROUTING -i $interface -p udp -m multiport --dport $merlinclash_nokpacl_default_port -j merlinclash_PREROUTING
								iptables -t mangle -A PREROUTING -i pptp+ -p udp -m multiport --dport $merlinclash_nokpacl_default_port -j merlinclash_PREROUTING
							fi
						else
		...
		if [ "$list_flag" == "2" ]; then
			if [ -n "$nokpacl_nu" ]; then
				for nokpacl in $nokpacl_nu; do
					...
					if [ "$proxy_mode" == "1" ] && [ "$ports" != "" ]; then
						...
						iptables -t mangle -A PREROUTING $(factor $macaddr "-m mac --mac-source") -i $interface -p tcp $(factor $ports "-m multiport --dport") -j merlinclash_PREROUTING
						iptables -t mangle -A PREROUTING $(factor $macaddr "-m mac --mac-source") -i pptp+ -p tcp $(factor $ports "-m multiport --dport") -j merlinclash_PREROUTING
						if [ "$ipv6_flag" == "1" ]; then
							...
							ip6tables -t mangle -A PREROUTING $(factor $macaddr "-m mac --mac-source") -i $interface -p tcp $(factor $ports "-m multiport --dport") -j merlinclash_PREROUTING
							ip6tables -t mangle -A PREROUTING $(factor $macaddr "-m mac --mac-source") -i pptp+ -p tcp $(factor $ports "-m multiport --dport") -j merlinclash_PREROUTING
						fi
						if [ "$tproxymode" == "tcpudp" ]; then
							...
							iptables -t mangle -A PREROUTING $(factor $macaddr "-m mac --mac-source") -i $interface -p udp $(factor $ports "-m multiport ! --dport") -j merlinclash_PREROUTING
							iptables -t mangle -A PREROUTING $(factor $macaddr "-m mac --mac-source") -i pptp+ -p udp $(factor $ports "-m multiport ! --dport") -j merlinclash_PREROUTING
							...
							if [ "$ipv6_flag" == "1" ]; then
								...
								ip6tables -t mangle -I PREROUTING $(factor $macaddr "-m mac --mac-source") -i $interface -p udp $(factor $ports "-m multiport ! --dport") -j merlinclash_PREROUTING
								ip6tables -t mangle -I PREROUTING $(factor $macaddr "-m mac --mac-source") -i pptp+ -p udp $(factor $ports "-m multiport ! --dport") -j merlinclash_PREROUTING
			...
			#2.白名单内再放行
				...
				if [ "$cirswitch" == "1" ]; then
					...
					iptables -t mangle -A PREROUTING -m set --match-set lan_whitelist src -i $interface -p tcp -j merlinclash_PREROUTING
					iptables -t mangle -A PREROUTING -m set --match-set lan_whitelist src -i pptp+ -p tcp -j merlinclash_PREROUTING
					if [ "$ipv6_flag" == "1" ]; then
						...
						ip6tables -t mangle -A PREROUTING -m set --match-set lan_whitelist src -i $interface -p tcp -j merlinclash_PREROUTING
						ip6tables -t mangle -A PREROUTING -m set --match-set lan_whitelist src -i pptp+ -p tcp -j merlinclash_PREROUTING
					fi
					if [ "$tproxymode" == "tcpudp" ]; then
						iptables -t mangle -A PREROUTING -m set --match-set lan_whitelist src -i br0 -p udp -j merlinclash_PREROUTING
						iptables -t mangle -A PREROUTING -m set --match-set lan_whitelist src -i tun+ -p udp -j merlinclash_PREROUTING
						...
						if [ "$ipv6_flag" == "1" ]; then
							...
							ip6tables -t mangle -A PREROUTING -m set --match-set lan_whitelist src -i $interface -p udp -j merlinclash_PREROUTING
							ip6tables -t mangle -A PREROUTING -m set --match-set lan_whitelist src -i pptp+ -p udp -j merlinclash_PREROUTING
						fi
					fi
				else
					iptables -t mangle -A PREROUTING -m set --match-set lan_whitelist src -i br0 -p tcp -j merlinclash_PREROUTING
				...
				if [ "$merlinclash_nokpacl_default_port" == "all" ] || [ "$merlinclash_nokpacl_default_port" == "" ] ; then
					...
					if [ "$merlinclash_nokpacl_default_mode" == "1" ]; then #剩余主机访问指定端口通过clash
						...
						if [ "$cirswitch" == "1" ]; then
							...
							iptables -t mangle -A PREROUTING -i $interface -p tcp -m multiport --dport $merlinclash_nokpacl_default_port -j merlinclash_PREROUTING
							iptables -t mangle -A PREROUTING -i pptp+ -p tcp -m multiport --dport $merlinclash_nokpacl_default_port -j merlinclash_PREROUTING
							iptables -t mangle -A PREROUTING -p tcp -j ACCEPT #兜底让流量不进入merlinclash链
							if [ "$ipv6_flag" == "1" ]; then
								...
								ip6tables -t mangle -A PREROUTING -i $interface -p tcp -m multiport --dport $merlinclash_nokpacl_default_port -j merlinclash_PREROUTING
								ip6tables -t mangle -A PREROUTING -i pptp+ -p tcp -m multiport --dport $merlinclash_nokpacl_default_port -j merlinclash_PREROUTING
								ip6tables -t mangle -A PREROUTING -p tcp -j ACCEPT #兜底让流量不进入merlinclash链
							fi
							if [ "$dnshijacksel" == "front" ]; then
								iptables -t nat -I PREROUTING -p udp --dport 53 -i $interface -j REDIRECT --to-port "${dnslistenport}" >/dev/null 2>&1
							...
							if [ "$tproxymode" == "tcpudp" ]; then
								...
								if [ "$ipv6_flag" == "1" ]; then
									...
									ip6tables -t mangle -A PREROUTING -i $interface -p udp -m multiport --dport $merlinclash_nokpacl_default_port -j merlinclash_PREROUTING
									ip6tables -t mangle -A PREROUTING -i pptp+ -p udp -m multiport --dport $merlinclash_nokpacl_default_port -j merlinclash_PREROUTING
									ip6tables -t mangle -A PREROUTING -p udp -j ACCEPT #兜底让流量不进入merlinclash链
								fi
							fi
						else
...
creat_router_ipset(){
	...
	if [ "$dnsgoclash" == "1" ]; then
		echo_date "设置路由走代理的IP集，目前为rawgit与letsencrypt" >> $LOG_FILE
		rm -rf /tmp/router_proxy.list
		rm -rf /tmp/clash_router_tmp.txt
		echo "create router hash:net family inet hashsize 1024 maxelem 65536" >/koolshare/res/ipset_router.ipset
		...
		cat /tmp/clash_router.txt | grep -n "^Address" | awk -F ":" '{print $3}' | awk -F " " '{print $1}' | grep -v "127.0.0.1" | grep -Eo '([^0-9]|\b)((1[0-9]{2}|2[0-4][0-9]|25[0-5]|[1-9][0-9]|[0-9])\.){3}(1[0-9][0-9]|2[0-4][0-9]|25[0-5]|[1-9][0-9]|[0-9])([^0-9]|\b)' >> /tmp/clash_router_tmp.txt
		#临时方法：
		nslookup letsencrypt.org 127.0.0.1:${dnslistenport} > /tmp/clash_router.txt
		cat /tmp/clash_router.txt | grep -n "^Address" | awk -F ":" '{print $3}' | awk -F " " '{print $1}' | grep -v "127.0.0.1" | grep -Eo '([^0-9]|\b)((1[0-9]{2}|2[0-4][0-9]|25[0-5]|[1-9][0-9]|[0-9])\.){3}(1[0-9][0-9]|2[0-4][0-9]|25[0-5]|[1-9][0-9]|[0-9])([^0-9]|\b)' >> /tmp/clash_router_tmp.txt
		awk '!/^$/&&!/^#/{printf("add router %s'" "'\n",$0)}' /tmp/clash_router_tmp.txt >>/koolshare/res/ipset_router.ipset
		ipset -! flush router 2>/dev/null
		ipset -! restore </koolshare/res/ipset_router.ipset 2>/dev/null
	fi
	jdqd=$(get jdqd_jd_enable)
	if [ "$jdqd" == "1" ]; then
		ipset add router 149.154.160.0/20 #检测到京东签到，写入TG IP段，使得TGBOT能用
	fi
	sed -i '/^ *$/d' /koolshare/merlinclash/yaml_basic/ipsetproxy.yaml
	sed -i '/^ *$/d' /koolshare/merlinclash/yaml_basic/ipsetproxyarround.yaml
	if [ -s "/koolshare/merlinclash/yaml_basic/ipsetproxy.yaml" ]; then
		echo_date "创建强制代理ipset集" >> $LOG_FILE
		rm -rf /tmp/ipset_proxy.list
		cp -rf /koolshare/merlinclash/yaml_basic/ipsetproxy.yaml /tmp/ipset_proxy.list 2>/dev/null
		...
		echo "create ipset_proxy hash:net family inet hashsize 1024 maxelem 65536" >/koolshare/res/ipset_proxy.ipset
		echo "create ipset_proxy6 hash:net family inet6" >/koolshare/res/ipset_proxy6.ipset
		cat /tmp/ipset_proxy.list | while read line
		do
			#先检测是否为IP格式
			detect_ip ${line}
			a=$?
			if [ "$a" == "4" ]; then
				...
				echo ${line} >> /tmp/clash_ipsetproxy6.txt
			else
				echo_date "不为IP格式，检查是否为域名地址" >> $LOG_FILE
				detect_domain ${line}
				if [ "$?" == "0" ]; then
					...
					cat /tmp/clash_ipsetproxy_tmp2.txt | while read line2
					do
						detect_ip ${line2}
						b=$?
						if [ "$b" == "4" ]; then
							#echo_date "为合法IPV4格式，进行处理" >> $LOG_FILE
							echo ${line2} >> /tmp/clash_ipsetproxy.txt
						elif [ "$b" == "6" ]; then
							#echo_date "为合法IPV6格式，进行处理" >> $LOG_FILE
							echo ${line2} >> /tmp/clash_ipsetproxy6.txt
						fi
					done
				else
		...
		awk '!/^$/&&!/^#/{printf("add ipset_proxy %s'" "'\n",$0)}' /tmp/clash_ipsetproxy.txt >>/koolshare/res/ipset_proxy.ipset
		awk '!/^$/&&!/^#/{printf("add ipset_proxy6 %s'" "'\n",$0)}' /tmp/clash_ipsetproxy6.txt >>/koolshare/res/ipset_proxy6.ipset
		ipset -! flush ipset_proxy 2>/dev/null
		ipset -! flush ipset_proxy6 2>/dev/null
		ipset -! restore </koolshare/res/ipset_proxy.ipset 2>/dev/null
		ipset -! restore </koolshare/res/ipset_proxy6.ipset 2>/dev/null
	fi
	if [ -s "/koolshare/merlinclash/yaml_basic/ipsetproxyarround.yaml" ]; then
		echo_date "创建强制绕行ipset集" >> $LOG_FILE
		...
		echo "create ipset_proxyarround hash:net family inet hashsize 1024 maxelem 65536" >/koolshare/res/ipset_proxyarround.ipset
		echo "create ipset_proxy6 hash:net family inet6" >/koolshare/res/ipset_proxyarround6.ipset
		cat /tmp/ipset_proxyarround.list | while read line
		do
			#先检测是否为IP格式
			detect_ip ${line}
			a=$?
			if [ "$a" == "4" ]; then
				...
				detect_domain ${line}
				if [ "$?" == "0" ]; then
					echo_date "为合法域名格式，进行处理" >> $LOG_FILE
					...
					cat /tmp/clash_ipsetproxyarround_tmp2.txt | while read line2
					do
						detect_ip ${line2}
						b=$?
						if [ "$b" == "4" ]; then
		...
		awk '!/^$/&&!/^#/{printf("add ipset_proxyarround %s'" "'\n",$0)}' /tmp/clash_ipsetproxyarround.txt >>/koolshare/res/ipset_proxyarround.ipset
		awk '!/^$/&&!/^#/{printf("add ipset_proxyarround6 %s'" "'\n",$0)}' /tmp/clash_ipsetproxyarround6.txt >>/koolshare/res/ipset_proxyarround6.ipset
		ipset -! flush ipset_proxyarround 2>/dev/null
		ipset -! restore </koolshare/res/ipset_proxyarround.ipset 2>/dev/null
		ipset -! flush ipset_proxyarround6 2>/dev/null
		ipset -! restore </koolshare/res/ipset_proxyarround6.ipset 2>/dev/null
	fi
}
load_nat() {
...
apply_nat_rules3() {
	...
	if [ "$tproxymode" == "closed" ] || [ "$tproxymode" == "udp" ]; then
		...
		if [ "$tproxymode" == "udp" ]; then
			...
			#ip集强制绕过
			iptables -t mangle -A merlinclash -p udp -m set --match-set ipset_proxyarround dst -j RETURN
			#局域网&排除地址绕行
		...
		iptables -t mangle -A PREROUTING -i $interface -p udp -j merlinclash_PREROUTING
		iptables -t mangle -A PREROUTING -i pptp+ -p udp -j merlinclash_PREROUTING
		...
		iptables -t nat -A PREROUTING -i $interface -p tcp -j merlinclash
		iptables -t nat -A PREROUTING -i pptp+ -p tcp -j merlinclash
		...
		if [ "$ipv6_flag" == "0" ]; then
			...
			iptables -t mangle -A PREROUTING -i $interface -p tcp -j merlinclash_PREROUTING
			iptables -t mangle -A PREROUTING -i pptp+ -p tcp -j merlinclash_PREROUTING
			iptables -t mangle -A PREROUTING -i br0 -p udp -j merlinclash_PREROUTING
			iptables -t mangle -A PREROUTING -i tun+ -p udp -j merlinclash_PREROUTING
			iptables -t mangle -A PREROUTING -i $interface -p udp -j merlinclash_PREROUTING
			iptables -t mangle -A PREROUTING -i pptp+ -p udp -j merlinclash_PREROUTING
	...
	# ipv6设置策略路由
		if [ "$ipv6_flag" == "1" ]; then
			...
			#强制转发clash
			ip6tables -t mangle -A PREROUTING -p tcp -m set --match-set ipset_proxy6 dst -j merlinclash_PREROUTING
			ip6tables -t mangle -A PREROUTING -p udp -m set --match-set ipset_proxy6 dst -j merlinclash_PREROUTING
			#强制绕行clash
			ip6tables -t mangle -A PREROUTING -p tcp -m set --match-set ipset_proxyarround6 dst -j RETURN
			ip6tables -t mangle -A PREROUTING -p udp -m set --match-set ipset_proxyarround6 dst -j RETURN
			#局域网&排除地址绕行
			...
			ip6tables -t mangle -A PREROUTING -i $interface -p tcp -j merlinclash_PREROUTING
			ip6tables -t mangle -A PREROUTING -i pptp+ -p tcp -j merlinclash_PREROUTING
			...
			ip6tables -t mangle -A PREROUTING -i $interface -p udp -j merlinclash_PREROUTING
			ip6tables -t mangle -A PREROUTING -i pptp+ -p udp -j merlinclash_PREROUTING
		...
		iptables -t mangle -A merlinclash -p tcp -m set --match-set ipset_proxy dst --syn -j MARK --set-mark 0x2333
		#ip集强制绕过
		iptables -t mangle -A merlinclash -p tcp -m set --match-set ipset_proxyarround dst -j RETURN
		#局域网&排除地址绕行
	...
	if [ "$dnsplan" == "fi" ]; then
		...
		ip6tables -t mangle -I PREROUTING -p udp --dport 53 -j DROP #阻断ipv6dns查询
	fi
...
apply_nat_rules4() {
	...
	if [ "$tproxymode" == "closed" ] || [ "$tproxymode" == "udp" ]; then
		...
		if [ "$p_lan_bypass_flag" == "0" ]; then
			...
			iptables -t nat -A PREROUTING -i $interface -p tcp -m set ! --match-set p_lan_bypass src -j merlinclash
			iptables -t nat -A PREROUTING -i pptp+ -p tcp -m set ! --match-set p_lan_bypass src -j merlinclash
		fi
		...
		if [ "$ipv6_flag" == "1" ]; then
			...
			iptables -t mangle -A PREROUTING -i br0 -p tcp -j merlinclash
			iptables -t mangle -A PREROUTING -i tun+ -p tcp -j merlinclash
			iptables -t mangle -A PREROUTING -i $interface -p tcp -j merlinclash
			iptables -t mangle -A PREROUTING -i pptp+ -p tcp -j merlinclash

			ip6tables -t mangle -A PREROUTING -i br0 -p tcp -j merlinclash
			ip6tables -t mangle -A PREROUTING -i tun+ -p tcp -j merlinclash
			ip6tables -t mangle -A PREROUTING -i $interface -p tcp -j merlinclash
			ip6tables -t mangle -A PREROUTING -i pptp+ -p tcp -j merlinclash
...
start_clash(){
	...
	#检查clash进程
	cdc=$(get merlinclash_check_delay_cbox)
	if [ "$cdc" == "1" ]; then

		delaytime=$(get merlinclash_check_delay_time)
...
start_custom(){
	...
	if [ "${LINUX_VER}" -lt "41" ]; then
		echo_date "检测到内核小于4.1，将Tproxy模式和IPV6代理强制关闭" >> $LOG_FILE
		tproxymode="closed"
		dbus set merlinclash_tproxymode=$tproxymode
		dbus set merlinclash_ipv6switch=0
	fi
...
apply_mc() {
	...
	echo_date --------------------- 开始设置dnsmasq的翻墙方案 开始------------------------ >> $LOG_FILE
	gen_gfw_too_dnsmasq
	echo_date --------------------- 开始设置dnsmasq的翻墙方案 结束------------------------ >> $LOG_FILE
	# 检测jffs2脚本是否开启
	detect
	...
	echo_date ""
	[ "$closeproxy" == "1" ] && echo_date --------------------- 创建fake-ip-range劫持------------------------ >> $LOG_FILE
	[ "$closeproxy" == "1" ] && update_dnsmasq_gfw_dns_ipt
	#----------------------------------KCP进程--------------------------------
